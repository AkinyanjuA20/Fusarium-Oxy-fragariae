...
for Transcriptome in $(ls gene_pred/final_genes/F.oxysporum_fsp_cepae/Fus2_canu_new/final/final_genes_appended_renamed.cdna.fasta); do
        Strain=$(echo $Transcriptome| rev | cut -d '/' -f3 | rev)
        Organism=$(echo $Transcriptome | rev | cut -d '/' -f4 | rev)
        echo "$Organism - $Strain"
        for RNADir in $(ls -d qc_rna/paired/F.oxysporum_fsp_cepae/Fus2_PDA); do
    ...
    # qc_rna/paired/F.oxysporum_fsp_cepae/Fus2_CzapekDox/F        6_S2_L001_R1_001_trim.fq.gz
    # qc_rna/paired/F.oxysporum_fsp_cepae/Fus2_CzapekDox/R        6_S2_L001_R2_001_trim.fq.gz
